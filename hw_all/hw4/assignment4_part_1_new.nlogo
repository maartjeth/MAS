...
to update-beliefs
 ...
 ; Please remember that you should use this method whenever your agents changes its position.
  ...
  let v 0
  ask vacuums[
      if observed_dirt != [] [                         ;//NEW
        foreach observed_dirt [                        ;//NEW
          let x item 0 ?                               ;//NEW
          let y item 1 ?                               ;//NEW
          if (member? (list x y) beliefs = false)[     ;//NEW
            set beliefs lput (list x y) beliefs        ;//NEW
          ]                                            ;//NEW
        ]                                              ;//NEW
        set observed_dirt []                           ;//NEW
      ]
      ...
      if beliefs != [] [
        ...
        if beliefs != [] [
          sort-beliefs
          ;set move_to_dirt item 0 beliefs //GONE
        ]
      ]
      set v v + 1
    ]
    ...
    ifelse desire = clean_all [
      ...
      if beliefs != [] [
        ifelse distancexy (item 0 item 0 beliefs) (item 1 item 0 beliefs) > 0.5 [
          ...
          [ set move_to_dirt item 0 beliefs ; // NEW
  ...
  ask patches in-radius vision_radius [
    if pcolor = clean_color [
      ...
      ask vacuums with [color = clean_color] [ ; bit strange that I call vacuum, patch, vacuum, but for as far as I know this is the only way to get this? Nicer solutions welcome :)
        ...
        ;if (member? (list x y) beliefs = false)[ // GONE
        ;  set beliefs lput (list x y) beliefs    //GONE
        ;]                                        //GONE
      ]
    ]
  ]
